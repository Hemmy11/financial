...
public class VerifyService {
    ...
    private static DateFormat DAY_FORMATE = new SimpleDateFormat("yyyy-MM-dd");

    private static String END_LINE = System.getProperty("line.separator","\n");

    private static DateFormat DATETIME_FORMATE = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    ...
    private Date getStartOfDay(Date day) {
        String start_str = DAY_FORMATE.format(day);
        Date start =null;
        try {
           start = DAY_FORMATE.parse(start_str);
    ...
    public File getPath(String rootDir,String chanId, Date day){
        String name = DAY_FORMATE.format(day)+"-"+chanId+".txt";
    ...
    public static VerificationOrder parseLine(String line){
        ...
        try {
            order.setCreateAt(DATETIME_FORMATE.parse(props[7]));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return order;
